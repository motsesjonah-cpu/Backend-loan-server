...
const supabase = createClient(
  'https://ynnirdonkknncbrwmize.supabase.co',
...
app.get('/api/admin/submissions', async (req, res) => {
  try {
    ...
    // Flatten the joined data so code_assigned is top-level
    const flat = (data || []).map(p => ({
      ...p,
      code_assigned: p.loan_requests?.code_assigned || null,
      loan_amount:   p.loan_requests?.amount       || null,
      loan_network:  p.loan_requests?.network      || null,
    }));
